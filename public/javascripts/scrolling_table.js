...
function add_droppable_row_for(recordId) {
  // YAHOO.util.Event.onDOMReady(function() { 
  //   new YAHOO.util.DDTarget('team_' + recordId + "_name");
  // });
  // Droppables.add('team_' + recordId + '_name', 
  //                { hoverclass:'hovering', 
  //                  onDrop:function(element) {
  //                    console.log('Dropped ' + idRegex.exec(element.id) + ' on ' + recordId);
  //                    // Droppables.remove('team_' + recordId + '_row');
  //                    // $('team_' + recordId).addClassName("disabled");
  //                    // new Effect.Opacity('team_' + recordId + '_row', { from: 1.0, to: 0.5, duration: 0.5 });
  //                    // new Ajax.Request('/admin/teams/merge?target_id=' + recordId, 
  //                    //                  { asynchronous:true, 
  //                    //                    evalScripts:true, 
...
function add_draggable_for(recordId, recordName) {
    YAHOO.util.Event.onDOMReady(function() { 
      ...
      d.onDragDrop = function(e, id) {
        ...
        if (y > recordsY + recordsDiv.getHeight()) {
          alert("below table");
        }
      }    
    });
  // new Draggable('team_' + recordId + '_name', { delay:100,
  //                                      scroll: $('records'),
  //                                      // ghosting: !Prototype.Browser.IE,
  //                                      superghosting: true,
  //                                      revert: true,
  //                                      onEnd:function() { add_droppable_row_for(recordId); },
  //                                      onStart:function() { Droppables.remove('team_' + recordId + '_name'); dragging = true;}
  //                                    }
  //               );
}
